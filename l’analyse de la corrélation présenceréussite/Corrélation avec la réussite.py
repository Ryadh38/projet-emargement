...
#Calcule la corrélation (force du lien) entre présence et notes

corr_presence_cc = np.corrcoef(stats['taux_presence'], stats['moyenne_cc'])[0, 1]
corr_presence_exam = np.corrcoef(stats['taux_presence'], stats['moyenne_exam'])[0, 1]

print("Corrélation taux de présence / note CC :", corr_presence_cc)
print("Corrélation taux de présence / note EXAM :", corr_presence_exam)

import matplotlib.pyplot as plt

plt.scatter(stats['taux_presence'], stats['moyenne_cc'], color='blue', label='Note CC')
plt.scatter(stats['taux_presence'], stats['moyenne_exam'], color='red', label='Note Exam')
plt.xlabel('Taux de présence')
plt.ylabel('Note')
plt.title('Présence vs. Réussite académique')
plt.legend()
plt.show()
